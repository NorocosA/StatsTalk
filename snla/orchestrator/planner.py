...
from . import GreylistPending, PlanResult
...
def _mock_intent(
    ...
    if any(w in text for w in (
        "非参数.*两组", "mann.*whitney", "曼惠特尼", "秩和.*两组",
    ...
    if any(w in text for w in (
        "非参数.*多组", "kruskal.*wallis", "克鲁斯卡尔", "秩和.*多组",
        "不服从正态.*多组", "非正态.*多组",
